...
SELECT MADT, TEN FROM SYS.DOITAC WHERE MANV = 1;
SELECT * FROM SYS.DOITAC WHERE MADT NOT IN (SELECT MADT FROM SYS.DOITAC WHERE MANV = 1) ORDER BY MADT;
UPDATE SYS.DOITAC SET MANV = 3 WHERE MADT = 1;

--select ds chuc vu
SELECT DISTINCT CHUCVU   FROM SYS.NHANVIEN;
--loc nhan vien theo chuc vu va theo chinhanh;
SELECT * FROM SYS.NHANVIEN WHERE MACN='1' ;
--select theo ten
SELECT * FROM SYS.NHANVIEN WHERE upper(HOTEN) like upper('%Thien c%');
