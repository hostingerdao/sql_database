...
--f.
--Gán quy?n SELECT, UPDATE, INSERT, DELETE d? li?u c?a b?ng Employee, Dependent cho HRmanager.
grant select, update, insert, delete on employee to HRmanager;
grant select, update, insert, delete on Dependent to HRmanager;

--g.
--Gán quy?n SELECT d? li?u c?a b?ng Employee, quy?n INSERT và UPDATE trên c?t Salary c?a b?ng Employee cho Accountant.
...
--h. Gán quy?n SELECT trên c?t Salary c?a b?ng Employee cho Accountant. Gi?i thích hi?n tý?ng.
...
--j. Gán quy?n SELECT trên b?ng Employee, quy?n UPDATE trên c?t Address c?a b?ng Employee cho
--HRemployee và cho phép HRemployee c?p l?i các quy?n ðó cho user khác.
...
--k. Dùng tài kho?n HRemployee c?p quy?n SELECT trên b?ng Employee, quy?n UPDATE trên c?t
...
--l. Thu h?i t?t c? các quy?n h? th?ng c?a Director. Admin có c?n quy?n t?o và xóa b?ng trong
--Company hay không?
