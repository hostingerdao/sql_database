--alter table employee drop constraint fk_emp_superssn;
--alter table employee drop constraint fk_emp_dept_dno;
--alter table department drop constraint fk_dept_emp_mgrssn;
--
--drop table employee;
--drop table department;
-----------------Create database for Lab 5-----------------------------------------------------
...
-----------trong oracle viec insert can check nen phai gan deferred, con viec delete thi deferred ko co huu ich gi het ma phu thuoc vao dieu kien
-----------co cho phep null hay khong?? neu khong cho phep thi khong the lam dc, hac la phai xoa het toan bo cascade ( nhung cac on tham khao cua no phai cho phep null)
-----------constraint khong the modify ma chi co the drop roi add lai
